  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200 transition-colors">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 text-center transition-colors">Cập nhật tài khoản</h3>
        ...
        {profileSuccess && (
          <div className="mb-4 p-3.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 text-xs font-semibold">
            ✅ {profileSuccess}
          </div>
        )}
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-4">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-[10px] text-slate-500 dark:text-slate-400 text-center transition-colors">Kích thước tối đa 2MB (Tải từ máy tính của bạn)</span>
          </div>
          ...
          {/* Email (Read-only) */}
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wider transition-colors">Email (Không thể thay đổi)</label>
            <input
              type="email"
              value={user?.email || ''}
              disabled
          ...
          {/* Password Input */}
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wider transition-colors">Mật khẩu mới (Để trống nếu giữ nguyên)</label>
            <input
              type="password"
